...
def get_sub_info_from_mkv(mkv_file):
	output = subprocess.run(["mkvmerge", "-i", mkv_file, "-F", "json"], capture_output=True)
	if output.stderr: print("STDERR", output.stderr)
	if output.stdout:
...
#	subprocess.run(["mkvextract", mkv_file, "tracks", "{}:{}.{}.srt".format(track_id, name, lang)])

if __name__ == "__main__":
	#mkv_path = "/Volumes/macOS HD/jose/Documents/prueba_mkv/Stranger Things 2x01 MADMAX [WEBRip 720p x264 Dual AC3 5.1 Subs][GrupoHDS].mkv"
	#extract_srt_from_mkv(mkv_file=mkv_path, track_id=3)
	mkv_path = sys.argv[1]
	#print(mkv_path)
	info = get_sub_info_from_mkv(mkv_file=mkv_path)
